...
describe("Minting", function () {
  ...
  describe("By normal users", function () {
    ...
    it("Should not mint if sale is not enabled", async function () {
      await expect(
        collection.mintTo(user.address, [dna1], true)
    ...
    it("Should throw on quantity 0", async function () {
      ...
      await expect(
        collection.mintTo(user.address, [], true)
    ...
    it("Tx value must cover the cost of the NFT", async function () {
      ...
      await expect(
        collection.connect(owner).mintTo(user.address, [dna1], true)
    ...
    it("quantity cannot be greater than the max allowed per transaction 'maxPerSale'", async function () {
      ...
      await expect(
        collection.connect(user).mintTo(user.address, dnaList, true, {value: payValue.toString()})
    ...
    it("should be able to Mint with ETH", async function () {
      ...
      tx = await collection.connect(user).mintTo(user.address, [dna1], true, {value: priceInEth.toString()})
      await tx.wait()
    ...
    it("should be able to Mint with 'approved' USDT", async function () {
      ...
      tx = await collection.connect(user).mintTo(user.address, [dna1], false);
      await tx.wait()
    ...
    it("should not be able to mint without 'approved' USDT", async function () {
      ...
      await expect(
        collection.connect(user).mintTo(user.address, [dna1], false)
    ...
    it("should not be able to Mint for someone else in public sale with ETH using the same DNA", async function () {
      ...
      await expect(
        collection.connect(user).mintTo(user.address, [dna1], true, {value: payValue.toString()})
